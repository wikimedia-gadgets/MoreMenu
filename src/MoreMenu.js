...
$(() => {
    ...
    function buildMenuVector(parentKey, html) {
        const $menu = $('#p-cactions').clone(true);
        if ($menu.length) {
            $menu.attr('id', `p-${parentKey}`);
            $menu.addClass([
                `mw-portlet-${parentKey}`,
                `mm-${parentKey}`,
                'mm-tab',
            ]).removeClass('mw-portlet-cactions').attr('aria-labelledby', `p-${parentKey}-label`);
            $menu.find('input[type="checkbox"]').attr('id', `p-${parentKey}-checkbox`);
            $menu.find('label').attr('id', `p-${parentKey}-label`);
